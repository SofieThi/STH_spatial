...
load("data/data_sogn.rda") #number of fitness centers in each parish and how many in the target group lives there
...
We do this by making a loop on Google maps, which search for a set of coordinates then scrape the address
...
print(paste0("Getting adress on ", i, " out of ", nrow(data)))
 Sys.sleep(3)
# Find the google maps search input field again and clear it:
webElem = remDr$findElement("xpath", "/html/body/div[3]/div[9]/div[3]/div[1]/div[1]/div[1]/div[2]/form/div/div[3]/div/input[1]")
...
Sys.sleep(3)
# Get the adress:
res = remDr$findElement('xpath', '//html/body/div[3]/div[9]/div[8]/div/div[1]/div/div/div[7]/div/div[1]/span[3]/span[3]')
...
I wrote the parish manually since it did not work to get them in a loop. The reason being the structure of the 'sogn.dk' website
```{r}
data$sogn[1] <- "tilst"
...
load("data_gym.rda")
#matched sogn/parish to fitness center and getting it into one dataset
```
...
Load the parish data from Danmarks statistik:
```{r}
library(readxl)
sogn <- read_excel("data/sogn.xlsx")
View(sogn)
...
sogndat$gyms[i] <- length(grep(sogndat$id[i], data$sogn))
}
...
# Getting (most of) the parishes in Aarhus
```
